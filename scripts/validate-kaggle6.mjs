...
const INPUT_PATH = path.resolve(process.cwd(), 'tmp/kaggle6-enriched.json')
const EXTERNAL_PATH = path.resolve(process.cwd(), 'src/data/externalArtData.json')
const OUTPUT_PATH = path.resolve(process.cwd(), 'tmp/kaggle6-validated.json')
const REPORT_DIR = path.resolve(process.cwd(), 'scripts/reports')
const REPORT_PATH = path.join(REPORT_DIR, 'kaggle6-validation-report.json')
const MIN_OVERALL_CONFIDENCE = Number(process.env.KAGGLE6_MIN_OVERALL_CONFIDENCE ?? 0.62)
const MAX_PER_CITY = Number(process.env.KAGGLE6_MAX_PER_CITY ?? 140)
...
async function main() {
  ...
  if (!records.length) throw new Error('No enriched Kaggle6 records. Run kaggle6:enrich first.')
  ...
  for (const item of records) {
    ...
    accepted.push({ ...item, lat, lng, region })
  }
  ...
  console.log(`Kaggle6 validation complete. Accepted ${accepted.length}, rejected ${rejected.length}`)

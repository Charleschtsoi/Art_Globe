...
const INPUT_PATH = path.resolve(process.cwd(), 'tmp/kaggle2-enriched.json')
const EXTERNAL_PATH = path.resolve(process.cwd(), 'src/data/externalArtData.json')
const OUTPUT_PATH = path.resolve(process.cwd(), 'tmp/kaggle2-validated.json')
const REPORT_DIR = path.resolve(process.cwd(), 'scripts/reports')
const REPORT_PATH = path.join(REPORT_DIR, 'kaggle2-validation-report.json')
const MIN_OVERALL_CONFIDENCE = Number(process.env.KAGGLE2_MIN_OVERALL_CONFIDENCE ?? 0.6)
const MAX_PER_CITY = Number(process.env.KAGGLE2_MAX_PER_CITY ?? 120)
...
async function main() {
  ...
  if (!records.length) throw new Error('No enriched Kaggle2 records. Run kaggle2:enrich first.')
  ...
  console.log(`Kaggle2 validation complete. Accepted ${accepted.length}, rejected ${rejected.length}`)

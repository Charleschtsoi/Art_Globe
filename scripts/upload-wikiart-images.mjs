...
const INPUT_PATH = path.resolve(process.cwd(), 'tmp/wikiart-validated.json')
const OUTPUT_PATH = path.resolve(process.cwd(), 'tmp/wikiart-uploaded.json')
const LOCAL_OUTPUT_DIR = path.resolve(process.cwd(), 'public/artworks/wikiart')
const MODE = String(process.env.WIKIART_UPLOAD_MODE ?? process.env.ARCHIVE3_UPLOAD_MODE ?? 'local').toLowerCase()
const CLOUDINARY_CLOUD_NAME = process.env.CLOUDINARY_CLOUD_NAME ?? ''
const CLOUDINARY_UPLOAD_PRESET = process.env.CLOUDINARY_UPLOAD_PRESET ?? ''
const CDN_BASE_URL = String(process.env.WIKIART_CDN_BASE_URL ?? process.env.ARCHIVE3_CDN_BASE_URL ?? '').replace(
  /\/$/,
  ''
)
...
async function uploadToCloudinary(buffer, fileName) {
  ...
  form.append('folder', 'art-globe/wikiart')
...
async function uploadLocal(buffer, fileName) {
  ...
  const relative = `/artworks/wikiart/${fileName}`
...
async function main() {
  ...
  if (!accepted.length) throw new Error('No validated WikiArt records. Run wikiart:validate first.')
  ...
  for (let index = 0; index < accepted.length; index += 1) {
    ...
    const fileName = `wikiart-${hash.slice(0, 20)}.${ext}`
    ...
    if ((index + 1) % 100 === 0) {
      console.log(`WikiArt upload progress: ${index + 1}/${accepted.length}`)
    }
  }
  ...
  console.log(`WikiArt image upload complete. Mode: ${mode}`)
